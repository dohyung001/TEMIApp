...
export default function MapAndSchedulePage() {
  ...
  const baseStyle =
    "w-full text-left px-6 py-4 rounded-xl text-xl font-bold transition-all duration-200";
  ...
  return (
    <div className="relative w-full h-full p-10 overflow-hidden">
      {/* ✅ 레이아웃 전체를 다시 덮지 않도록 w-screen / h-screen / bg 제거 */}
      ...
      <div className="flex w-full h-full pt-0">
        ...
        <nav className="w-64 flex-shrink-0 flex flex-col gap-4">
          <button
            ...
            className={`${baseStyle} ${
              activeView === "map" ? activeStyle : inactiveStyle
            }`}
          >
            지도
          </button>
          <button
            ...
            className={`${baseStyle} ${
              activeView === "schedule" ? activeStyle : inactiveStyle
            }`}
          >
            경진대회 정보
          </button>
        ...
        <main className="flex-grow ml-8 bg-white rounded-2xl shadow-lg overflow-hidden p-0">
          {/* ---------------------------------------------------- */}
          {/* ★★★ 2-1. '지도' 탭 선택 시 ★★★ */}
          {/* ---------------------------------------------------- */}
          {activeView === "map" && (
